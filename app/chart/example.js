// https://github.com/rayanfer32/nexus-explorer-next/blob/6de445ad747a0b1ad4bdbd588061afa4b2835c09/src/components/common/NE_Chart/ChartApexArea/apex_area.jsx
...
function AreaChart({ initialData }) {
  ...
  const { isLoading, data } = useQuery(['charting', limit, network.name], () => getRecentBlocks(limit), {
    initialData: limit <= 120 && network.name == NETWORKS.MAINNET.name ? initialData : undefined,
  });
    ...
    initialEssentials({
      options: {
        ...
        fill: {
          type: 'gradient',
          gradient: {
            gradientToColors: isDarkMode ? [TYPES.COLORS.SKY_BLUE] : [TYPES.COLORS.NEXUS_BLUE],
          },
          opacityFrom: 0.7,
          opacityTo: 0.3,
          stops: [0, 90, 100],
  ...
  useEffect(() => {
    ...
    setChartState((prev) => {
      (prev.options.chart.toolbar.show = windowSize.width > 512), (prev.options.theme.mode = sharedState.theme);
      // update colors property of the chart
      prev.options.colors = isDark ? [TYPES.COLORS.SKY_BLUE] : [TYPES.COLORS.NEXUS_BLUE];
      // update fill color of the chart
      prev.options.fill.gradient.gradientToColors = isDark ? [TYPES.COLORS.SKY_BLUE] : [TYPES.COLORS.NEXUS_BLUE];
      return { ...prev };
    });
